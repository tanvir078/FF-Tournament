...
import { Banner, useBanners } from '@/views/user/Mobile/_components/tournaments';
...
const topTournaments = ['free fire', 'pubg', 'valorant', 'cs2', 'apex legends', 'Dream League Soccer', 'Dota 2'];

export const topTournament =[
  {id: 1, label: 'free fire', value: '', title: ''},
  {id: 2, label: 'pubg', value: '', title: ''},
  {id: 3, label: 'valorant', value: '', title: ''},
  {id: 4, label: 'cs2', value: '' as string, title: ''},
  {id: 5, label: 'apex legends', value: '', title: ''},
  {id: 6, label: 'Football', value: '', title: ''},
  {id: 7, label: 'Dota 2', value: '', title: ''},
]
...
export default function Dashboard() {
  ...
  const { banners } = useBanners(data.featured);
  ...
  if (!hydrated || !user || loading) {
    return (
      <div className="min-h-screen bg-[#67078cd1] flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#0671a2d1]"></div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div>
        ...
        <section className="fixed inset-x-0 top-12 z-10 h-[190px] overflow-hidden rounded-bl-[25px] rounded-br-[25px] bg-gray-950 text-white shadow-lg md:hidden">
          <DashboardImageCarousel banners={banners} />
        </section>
          
          {/* Spacer to push content below the fixed header */}
           <div className="h-[190px] md:hidden" />
          
          <div className="relative z-20 bg-bg2 rounded-t-[35px] md:static">
          <MobileTournamentRail
            tournaments={data.featured}
            openTournament={(id) => router.push(`/user/tournaments/${id}`)}
            browseAll={() => router.push('/user/tournaments')}
          />

          {/* Tab navigation */}
          <div className="mb-2 grid h-[50px] grid-cols-3 border-b border-gray-800 pb-px sm:gap-2 sm:pt-">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-3">
              ...
              {activeTournament && (
                <Card className="p-4 border-l-4 border-l-primary-500 bg-gradient-to-r from-primary-500/5 to-transparent">
...
function DashboardImageCarousel({ banners }: { banners: Banner[] }) {
  const router = useRouter();
    ...
    <div className="scrollbar-hide flex h-full snap-x snap-mandatory overflow-x-auto">
      {banners.map((banner) => (
        <button
          ...
          className="relative h-[190px] w-screen min-w-full snap-start overflow-hidden bg-gray-900"
...
function MobileTournamentRail({
  ...
  return (
    <section className="md:hidden border-b border-white/10 bg-gray-950 px-4 pb-4 pt-3">
      <div className="mb-2 flex items-center justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-300">Tournaments</p>
        </div>
        <button onClick={browseAll} className="flex items-center gap-1 text-[11px] font-bold text-primary-300">
          View all <ChevronRight className="h-3 w-3" />
        </button>
      </div>

      <div className="scrollbar-hide flex gap-3 overflow-x-auto pb-1 pr-2">
        {tournaments.slice(0, 8).map((tournament) => (
          <button
            key={tournament.id}
            onClick={() => openTournament(tournament.id)}
            className="relative h-[118px] w-[174px] flex-none overflow-hidden rounded-2xl border border-white/10 bg-gray-900 text-left shadow-lg shadow-black/20"
          >
            {(tournament.banner || tournament.game?.banner) && (
              <img
                src={tournament.banner || tournament.game?.banner}
                alt=""
            ...
            <div className="relative z-10 flex h-full flex-col justify-end p-3">
              <div className="mb-1 flex items-center justify-between gap-2">
                <span className="truncate text-[10px] font-bold uppercase text-primary-200">
              ...
              <h3 className="line-clamp-2 text-xs font-black leading-4 text-white">
                {tournament.title || tournament.name}
              </h3>
              <div className="mt-2 flex items-center justify-between gap-2 text-[10px] text-gray-300">
                <span className="truncate">{tournament.gameMode?.name || tournament.format || 'Mode'}</span>
                <span className="shrink-0">{Number(tournament.entryFee || 0) === 0 ? 'Free' : `৳${tournament.entryFee}`}</span>
              </div>
            </div>
          </button>
        ))}
