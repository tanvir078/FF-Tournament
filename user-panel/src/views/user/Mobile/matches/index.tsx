  ...
  return (
    <div className="min-h-full bg-gray-950 text-white">
      <div className="space-y-5 px-4 py-4">
        <header className="rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(239,68,68,0.22),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(17,24,39,0.9))] p-5">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-red-300">Match Center</p>
          <h1 className="mt-2 text-2xl font-black">Rooms, Schedule, Claims</h1>
          <p className="mt-2 text-sm leading-6 text-gray-400">Follow upcoming matches, reveal eligible room credentials, and submit result claims after completion.</p>
        </header>
        ...
        {matches.length ? (
          ...
          <Card className="border-white/10 bg-gray-900/80 p-8 text-center">
            <Calendar className="mx-auto mb-4 h-14 w-14 text-gray-600" />
            <h3 className="text-xl font-black text-gray-200">No matches yet</h3>
            <p className="mt-2 text-sm text-gray-500">Join a tournament and approved matches will appear here.</p>
...
function MatchCard({
  ...
  return (
    <Card className="border-white/10 bg-gray-900/80 p-4">
      {claim && (
        <div className={`mb-4 rounded-xl border px-3 py-2 text-xs font-bold ${claimTone(claim.status)}`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-black text-gray-300">{formatMeta(match.stage)}</span>
        <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-black text-gray-300">{match.format}</span>
      </div>

      <h3 className="mt-3 line-clamp-2 text-lg font-black">{match.tournamentName}</h3>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs font-medium text-gray-400">
        <span><Calendar className="mr-1 inline h-3.5 w-3.5" />{new Date(match.scheduledTime).toLocaleDateString()}</span>
        <span><Clock3 className="mr-1 inline h-3.5 w-3.5" />{new Date(match.scheduledTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
      </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`rounded-full border px-2.5 py-1 text-[11px] font-black ${tone}`}>{status === 'IN_PROGRESS' && <Flame className="mr-1 inline h-3 w-3" />}{status.replace(/_/g, ' ')}</span>;
}
