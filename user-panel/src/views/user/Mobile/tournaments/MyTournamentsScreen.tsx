...
import { Calendar, Clock3, Eye, EyeOff, Flame, Lock, Trophy, Users } from 'lucide-react';
  ...
  return (
    <div className="min-h-full bg-gray-950 text-white">
      <div className="space-y-5 px-4 py-4">
        <header className="rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.24),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(17,24,39,0.9))] p-5">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-primary-300">My Tournaments</p>
          <h1 className="mt-2 text-2xl font-black">Tournament Hub</h1>
          <p className="mt-2 text-sm leading-6 text-gray-400">Track joined events, check-in, live rooms, brackets, and completed runs from one flow.</p>
        </header>
        ...
        {tournaments.length ? (
          ...
          <Card className="border-white/10 bg-gray-900/80 p-8 text-center">
            <Trophy className="mx-auto mb-4 h-14 w-14 text-gray-600" />
            <h3 className="text-xl font-black text-gray-200">No joined tournaments</h3>
            <p className="mt-2 text-sm text-gray-500">Join an open tournament to start tracking your flow here.</p>
            <Button className="mt-5" onClick={() => router.push('/user/tournaments')}>Browse Tournaments</Button>
          </Card>
        )}
      </div>
    </div>
...
function TournamentCard({
  ...
  return (
    <Card className="overflow-hidden border-white/10 bg-gray-900/80 p-4">
      ...
      <h3 className="mt-3 line-clamp-2 text-lg font-black">{item.title || item.name}</h3>
      <p className="mt-1 text-sm text-primary-300">{item.game?.name || 'Game'} · {item.gameMode?.name || item.format || 'Mode'}</p>

      <div className="mt-4 grid grid-cols-2 gap-3 text-xs font-medium text-gray-400">
        <span><Calendar className="mr-1 inline h-3.5 w-3.5" />{item.startDate ? new Date(item.startDate).toLocaleDateString() : 'TBA'}</span>
        <span><Clock3 className="mr-1 inline h-3.5 w-3.5" />{item.startTime || 'TBA'}</span>
        <span><Trophy className="mr-1 inline h-3.5 w-3.5" />৳{Number(item.prizePool || 0).toLocaleString()}</span>
        <span><Users className="mr-1 inline h-3.5 w-3.5" />{item.format || 'Format'}</span>
      </div>

      {item.roomDetails?.roomId && (
        <div className="mt-4 rounded-2xl border border-primary-400/20 bg-primary-500/10 p-3">
          <div className="mb-2 flex items-center gap-2 text-sm font-bold text-primary-300"><Lock className="h-4 w-4" />Private room</div>
          ...
          <button onClick={toggleRoom} className="mt-2 inline-flex items-center gap-2 text-xs font-bold text-gray-300">
      ...
      <Button className="mt-4 w-full" variant={item.checkInStatus === 'PENDING' ? 'primary' : 'outline'} onClick={item.checkInStatus === 'PENDING' ? checkIn : open} disabled={checkingIn}>
        {linkLabel}
      </Button>
    </Card>
  );
