...
export default function DesktopBracketTree({
  tournament,
  matches,
}: {
  ...
  return (
    <div className="-m-6 space-y-4">
      <div className="border-y border-white/10 bg-[#070b14]">
        <div className="scrollbar-hide overflow-x-auto px-6 py-6">
          <div className="flex min-w-max gap-8">
            {stages.map((stage, stageIndex) => (
              <section key={stage} className="w-[300px] flex-none">
                <div className="mb-5 flex h-9 items-center justify-between border-b border-white/10">
                  <h3 className="text-xs font-black uppercase tracking-[0.18em] text-gray-300">{stageLabel(stage)}</h3>
                ...
                <div className="relative space-y-5">
                  {grouped[stage].map((match, matchIndex) => (
                    <BracketNode
                      ...
                      offset={stageIndex * 28 + (matchIndex % 2) * 18}
                    />
                  ))}
                </div>
              </section>
      ...
      <div className="grid gap-4 px-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)_minmax(280px,0.8fr)]">
        <section className="border-y border-white/10 bg-[#070b14] px-4 py-4">
          <div className="mb-3 flex items-center justify-between">
            <span className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-gray-300"><Users className="h-4 w-4 text-primary-300" /> Seeds</span>
          ...
          <div className="grid gap-2 xl:grid-cols-2">
            {(entrants.length ? entrants : [{ id: 'seed-pending', name: 'Seed pending' }]).map((entrant, index) => (
              <div key={`${entrant.id}-${index}`} className="flex h-10 min-w-0 items-center gap-2 border border-white/10 bg-white/[0.03] px-3">
                <span className="w-6 shrink-0 text-[11px] font-black text-gray-500">{index + 1}</span>
                <span className="truncate text-sm font-bold text-gray-200">{entrant.name}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="border-y border-white/10 bg-[#070b14] px-4 py-4">
          <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-gray-300"><ShieldCheck className="h-4 w-4 text-emerald-300" /> Eligibility</div>
          <div className="flex flex-wrap gap-2">
            {eligibility.map((item) => <span key={item} className="border border-white/10 bg-white/[0.03] px-3 py-1.5 text-sm font-bold text-gray-300">{item}</span>)}
          </div>
        </section>

        <section className="border-y border-white/10 bg-[#070b14] px-4 py-4">
          <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-gray-300"><Calendar className="h-4 w-4 text-sky-300" /> Timeline</div>
          <div className="space-y-2">
            {timeline.map((item) => (
              <div key={item.label} className="flex items-center justify-between gap-3 border-b border-white/5 pb-2 text-sm last:border-0 last:pb-0">
...
function BracketNode({ match, connectOut, offset }: { match: BracketMatch; connectOut: boolean; offset: number }) {
  ...
  return (
    <div className="relative" style={{ paddingTop: offset }}>
      {connectOut && (
        <>
          <div className="absolute right-[-32px] top-[calc(50%+12px)] h-px w-8 bg-white/20" />
          <div className="absolute right-[-32px] top-[calc(50%+12px)] h-10 w-px bg-white/10" />
        </>
      )}

      <div className="overflow-hidden rounded-lg border border-white/10 bg-[#0c1220] shadow-[0_14px_40px_rgba(0,0,0,0.28)]">
        <div className="flex h-8 items-center justify-between border-b border-white/10 px-3">
          <span className="text-[11px] font-black text-gray-400">M{match.matchNumber || '-'}</span>
          <span className={`h-2.5 w-2.5 rounded-full ${statusDot(match.status)}`} />
        </div>
        ...
        {slots.map((slot, index) => {
          ...
          return (
            <div
              ...
              className={`flex h-11 items-center justify-between gap-3 border-b border-white/5 px-3 last:border-b-0 ${
                won ? 'bg-emerald-400/10 text-white' : 'text-gray-300'
              }`}
            >
              <span className={`min-w-0 truncate text-sm font-bold ${isTbd(slot) ? 'text-gray-500' : ''}`}>{slot.teamName || slot.name || 'TBD'}</span>
              {won && <Crown className="h-4 w-4 shrink-0 text-yellow-300" />}
            </div>
          );
        })}

        <div className="flex h-8 items-center justify-between gap-3 border-t border-white/10 px-3 text-[11px] font-bold text-gray-500">
...
function BracketEmpty({ title }: { title: string }) {
  return (
    <div className="-m-6 grid min-h-[360px] place-items-center border-y border-white/10 bg-[#070b14] px-6 text-center">
      <div>
        <GitBranch className="mx-auto mb-4 h-12 w-12 text-gray-600" />
        <h3 className="text-sm font-black uppercase tracking-[0.18em] text-gray-300">{title}</h3>
      </div>
    </div>
  );
}
