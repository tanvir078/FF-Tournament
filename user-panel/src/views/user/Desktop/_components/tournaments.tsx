...
export function TournamentCard({
  ...
  return (
    ...
    >
      <div className={`${compact ? 'h-[120px]' : 'h-36'} relative overflow-hidden bg-gray-800`}>
        {image ? (
      ...
      <div className={`${compact ? 'p-3.5' : 'p-4'}`}>
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="truncate text-xs font-black uppercase tracking-wide text-primary-300">
            {tournament.game?.name || 'Tournament'}
          </span>
          <span className="shrink-0 rounded-full bg-white/5 px-2.5 py-1 text-[11px] font-bold text-gray-300">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-medium text-gray-400">
          <span className="min-w-0 truncate"><Users className="mr-1 inline h-3.5 w-3.5" />{registeredTeams}/{maxTeams || '-'}</span>
          ...
          <span className="min-w-0 truncate"><Calendar className="mr-1 inline h-3.5 w-3.5" />{formatDate(tournament.startDate)}{tournament.startTime ? `, ${tournament.startTime}` : ''}</span>
        </div>

        <span className="mt-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-wide text-primary-300">
          {cta}
